...
public class PeerDiscoveryThread implements Runnable {
    ...
    public PeerDiscoveryThread(final Peer peer) {
    ...
    public void tryConnect(final Peer peer) {
        ...
        PeerDiscoveryThread.logger.info("Start to discory connectable peers");
        ...
        try {
            ...
            ch.addListener(new ChannelFutureListener() {
                ...
                public void operationComplete(final ChannelFuture future) throws Exception {
                    if (future.isSuccess()) {
                        PeerDiscoveryThread.logger.info("Connect to {}:{} successfully.", peer.getAddress().getHostName(), peer
                            .getAddress().getPort());
                    } else {
                        PeerDiscoveryThread.logger.info("Failed to connect to {}:{}", peer.getAddress().getHostName(), peer
                            .getAddress().getPort());
                    }
                }
            ...
            PeerDiscoveryThread.logger.error("failed to connect to {}:{}", peer.getAddress().getHostName(), peer.getAddress()
                .getPort());
